...
def main(train_data, train_targets, test_data, test_targets):
    ...
    conf_matrix = confusion_matrix(
        y_true=(test_targets.reshape(-1)), y_pred=(np.argmax(results, axis=-1)))

        
    conf_matrix2 = confusion_matrix(
        y_true=(test_targets.reshape(-1)), y_pred=thresh_results)
    conf_matrix3 = confusion_matrix(
        y_true=(test_targets.reshape(-1)), y_pred=thresh_results2)
    conf_matrix4 = confusion_matrix(
        y_true=(test_targets.reshape(-1)), y_pred=thresh_results3)
    conf_matrix5 = confusion_matrix(
        y_true=(test_targets.reshape(-1)), y_pred=thresh_results4)
